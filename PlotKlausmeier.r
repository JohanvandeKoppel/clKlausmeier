...
for (jj in 0:(NumFrames-1)){  # Here the time loop starts 
   ...
   image.plot(Data_N[,2:(NX-1)], zlim=c(0,20), xaxt="n", yaxt="n",
              ...
              legend.args=list(text=expression(Dry~weight~(g~m^-2)),
                               cex=0.8, line=0.5))  
   title("Vegetation biomass")   

   mtext(text=sprintf("Time : %1.0f of %1.0f years", (jj+1)/NumFrames*EndTime, EndTime), 
